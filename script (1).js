(function () {
  ...
  function renderFinishedList() {
    const finished = sorted.filter((r) => r.status === "finished");

    if (finished.length === 0) {
      finishedList.innerHTML = `
        <h2 class="ticket-title">Finished</h2>
        <p class="np-empty">Nothing finished yet.</p>
      `;
      return;
    }

    const byScore = [...finished].sort((a, b) => b.rating - a.rating);
    ...
    finishedList.innerHTML =
      ...
      byScore
        .map(
          (r) => `
        <div class="finished-item" data-id="${r.id}">
          <span class="fi-title">${escapeHtml(r.title)}</span>
          <span class="fi-score">${r.rating}<span class="fi-score-max">/10</span></span>
        </div>
      `
        )
        .join("");

    finishedList.querySelectorAll(".finished-item").forEach((el) => {
      el.addEventListener("click", () => {
        const match = REVIEWS.find((r) => r.id === el.dataset.id);
